...
struct SurfKey {
	...
	void fill(void* instance, SurfPublicData fData, real currentTime, const TTransform3D& ipL2G)
	{
	...
	boolean  operator==	(const SurfKey& rhs) 
	{ 
...
extern DCGSharedCache<TerrainToolsCache, ShadingIn, SurfKey> surfCache;

extern const MCGUID CLSID_Surf;
extern const MCGUID IID_Surf;

class Surf : public TBasicShader, public cTransformer, public IExStreamIO, public DCGShaderHelper
	, public InfinitePlaneEdgeHelper, public DCGBezier
	, public DCGSharedCacheClient<TerrainToolsCache, ShadingIn, SurfKey>
{
	public:
		Surf();
		~Surf();
		...
		virtual int32   MCCOMAPI GetParamsBufferSize    ()
			const        { return sizeof(SurfPublicData); }
	...
	protected:
		...
		void CopyDataFrom(const Surf* source);
