...
const MCGUID CLSID_MultiAdd(R_CLSID_MultiAdd);

real MultiAdd::GetValue(real& result,boolean& fullArea,ShadingIn& shadingIn)
{
...
real MultiAdd::GetColor(TMCColorRGBA& result,boolean& fullArea,ShadingIn& shadingIn)
{
	if (extensionDataChangeDirty)
		getChildShaders();
	real alpha = 0;
...
real MultiAdd::GetVector(TVector3& result,ShadingIn& shadingIn)
{
	if (extensionDataChangeDirty)
		getChildShaders();
	real alpha = 0;
